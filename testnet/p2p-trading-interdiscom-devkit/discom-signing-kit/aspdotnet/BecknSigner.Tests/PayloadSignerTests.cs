...
public class PayloadSignerTests
{
  ...
  ""message"": {
    ...
    private PayloadSigner CreateSigner() => new(new SignerConfig
    {
    ...
    {
        var (sub, key, alg) = PayloadVerifier.ParseKeyId("p2p-trading-sandbox1.com|76EU8aUq|ed25519");
        Assert.Equal("p2p-trading-sandbox1.com", sub);
        Assert.Equal("76EU8aUq", key);
        Assert.Equal("ed25519", alg);
