#!/usr/bin/env python3
"""
DEG Postman Collection Generator

Builds Postman collections from example JSON flows for the energy-data-exchange devkit.
Adapted from the DDM generate_postman_collection.py script.
...
def generate_collection(role: str, repo_root: Path):
    ...
    if role == "BAP":
        ...
        collection_name = "energy-data-exchange.BAP-DEG"
        desc = "Postman collection for Buyer Application Platform implementing energy-data-exchange DEG APIs based on Beckn Protocol v2"
    else:
        action_mapping = {**BPP_ACTIONS, **BPP_INITIATED_ACTIONS}
        adapter_url_var = "bpp_adapter_url"
        collection_name = "energy-data-exchange.BPP-DEG"
        desc = "Postman collection for Beckn Provider Platform implementing energy-data-exchange DEG APIs based on Beckn Protocol v2"
    ...
    variables = [
        ...
        {"key": "transaction_id", "value": "b2c3d4e5-f6a7-8901-bcde-f12345678901"},
        {"key": "iso_date", "value": ""},
    ]
    if role == "BAP":
...
def main():
    parser = argparse.ArgumentParser(description="Generate Postman collection from DEG example JSONs")
