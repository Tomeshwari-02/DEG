...
type Config struct {
	...
	// PolicyUrls is a list of URLs pointing to .rego files, fetched at startup.
	// Comma-separated in config. Supports http:// and https://.
	PolicyUrls []string

	// Query is the Rego query that returns a set of violation strings.
	// Default: "data.deg.policy.violations"
	Query string
