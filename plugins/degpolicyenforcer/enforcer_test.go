...
func TestParseConfig_Defaults(t *testing.T) {
	...
	if cfg.Query != "data.deg.policy.violations" {
...
`
	...
	eval, err := NewEvaluator(dir, "", nil, "data.deg.policy.violations", nil)
	if err != nil {
...
`
	dir := writePolicyDir(t, "test.rego", policy)
	eval, err := NewEvaluator(dir, "", nil, "data.deg.policy.violations", map[string]string{"maxValue": "100"})
	if err != nil {
		t.Fatalf("NewEvaluator failed: %v", err)
	}
...
`
	...
	eval, err := NewEvaluator("", "", []string{srv.URL + "/test_policy.rego"}, "data.deg.policy.violations", nil)
	if err != nil {
...
func TestEvaluator_FetchURL_NotFound(t *testing.T) {
	...
	_, err := NewEvaluator("", "", []string{srv.URL + "/missing.rego"}, "data.deg.policy.violations", nil)
	if err == nil {
...
func TestEvaluator_FetchURL_InvalidScheme(t *testing.T) {
	_, err := NewEvaluator("", "", []string{"ftp://example.com/policy.rego"}, "data.deg.policy.violations", nil)
	if err == nil {
...
`
	...
	eval, err := NewEvaluator(dir, "", []string{srv.URL + "/remote.rego"}, "data.deg.policy.violations", nil)
	if err != nil {
		t.Fatalf("NewEvaluator failed: %v", err)
	}
...
// --- Enforcer Integration Tests (with real delivery_window.rego) ---

func TestEnforcer_Compliant(t *testing.T) {
	policyDir := projectPolicyDir()
	if _, err := os.Stat(policyDir); os.IsNotExist(err) {
		t.Skip("project policy dir not found, skipping integration test")
	}
	...
	body := `{
		"context": {"timestamp": "2026-01-09T00:00:00Z", "action": "confirm"},
...
func TestEnforcer_SkipsNonMatchingAction(t *testing.T) {
	...
	body := `{
		"context": {"timestamp": "2026-01-09T00:00:00Z", "action": "search"},
		"message": {"order": {"beckn:orderItems": [{
			"beckn:acceptedOffer": {"beckn:offerAttributes": {"deliveryWindow": {
				"schema:startTime": "2026-01-09T01:00:00Z",
				"schema:endTime": "2026-01-09T05:00:00Z"
